...
def index(request, year_num, month_num):
    qs_inc = Personal_income.objects.all()
    qs_exp = Personal_expense.objects.all()
    ...
    progress = {}
    progress['color'] = {}
    if int(requested_monthly_expense) > last_monthly_expense:
    ...
    progress['difference'] = abs(int(requested_monthly_expense) -
                                 last_monthly_expense)
    tot_bud=total_budget-requested_monthly_expense
    data = {
        # 'expenses': curr_expenses,
        'prev': prev,
        'curr_monthly_expense': int(requested_monthly_expense),
...
def image(request):
    if request.method == "POST":
        ...
        import requests
        import base64
        CLIENT_ID = "vrfADx4iZiImciC0aKwDHhiwH5UTinyy2FbeN1L"
        ENVIRONMENT_URL = "https://api.veryfi.com/"

        username = "pranjalravipatil"
        api_key = "1902afdc318947d562dbbb984143d40c"
        process_image_url = '{0}api/v7/partner/documents/'.format(ENVIRONMENT_URL)
        headers = {
...
# file path and file name
        print(image)
        image_path = 'C:/Users/pranj/Downloads/'+str(image)
        file_name = str(image)
...
# Veryfi will try to choose the best one that fits this file
        ...
        new_expense = Expense(title="food",
                              currency = curr,
                              description="receipt payment",
                              amount=price,
                              payment_time=timezone.now(),
                              dateOfPayment = r['total']
                              )
